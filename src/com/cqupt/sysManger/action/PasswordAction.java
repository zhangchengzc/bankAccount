...
public class PasswordAction extends ActionSupport{
	...
	private String getData() {
		...
		try {
			...
			sql = "select user_id from sys_user where ";
			if(!passWord.equals(""))
			{
				sql+="'"+md5fun.getMD5ofStr(passWord)+"' = (select user_pwd from sys_user where user_id = '"+userId+"')";
			}
			logger.info("密码核实查询："+sql);
