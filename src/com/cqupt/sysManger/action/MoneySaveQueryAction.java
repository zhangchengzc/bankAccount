...
public class MoneySaveQueryAction extends ActionSupport{
	...
	private char[] getUserList(String deptId ,String pageSize, String page){
				...
				try{
					...
					sql = " select b.* , ROWNUM order_id FROM (select @rownum:=@rownum+1 as rownum,t.id , DATE_FORMAT(t.date,'%Y-%m-%d') date , t.card_number , t.save_money  from (select @rownum:=0) r, bankaccount.money_save t  )b  ";
